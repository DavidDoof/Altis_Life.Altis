...
#define Btn8 37457
#define Title 37401
private ["_Btn8","_display","_curTarget","_Btn1","_Btn2","_Btn3","_Btn4","_Btn5","_Btn6","_id","_Btn7"];
if (!dialog) then {
...
if (playerSide isEqualTo west) then {
	...
	_Btn7 ctrlSetText "Kennzeichen";
    _Btn7 buttonSetAction "
		private['_kfz','_txt'];
		_kfz = cursorObject getVariable['kfz',[]];
		if(count(_kfz) == 0)
		then
		{
			_txt = 'Kein Kennzeichen';
		}
		else
		{
			_txt = format['%1-%2-%3',_kfz select 0,_kfz select 1,_kfz select 2];
		};
		hint format['%1',_txt]; 
		closeDialog 0;
	";
