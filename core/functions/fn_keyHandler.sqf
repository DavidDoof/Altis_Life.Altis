...
*/
private ["_curObject","_handled","_shift","_alt","_code","_ctrl","_alt","_ctrlKey","_veh","_locked","_interactionKey","_mapKey","_interruptionKeys"];
...
switch (_code) do {
    ...
    case 19: {
        ...
        if (_shift && (playerSide isEqualTo west || (playerSide isEqualTo civilian && license_civ_rebel)) && {!isNull cursorObject} && {cursorObject isKindOf "Man"} && {(isPlayer cursorObject)} && {(side cursorObject in [civilian,independent,west])} && {alive cursorObject} && {cursorObject distance player < 3.5} && {!(cursorObject getVariable "Escorting")} && {!(cursorObject getVariable "restrained")} && {speed cursorObject < 1}) then {
            [] call life_fnc_restrainAction;
        };
		if(!_shift && _alt && !_ctrlKey && playerSide isEqualTo civilian && license_civ_rebel && {!isNull cursorObject} && {cursorObject isKindOf "Man"} && {(isPlayer cursorObject)} && {(side cursorObject in [civilian,independent,west])} && {alive cursorObject} && {cursorObject distance player < 3.5} && {!(cursorObject getVariable "Escorting")} && {(cursorObject getVariable "restrained")} && {speed cursorObject < 1}) then {
            [] call life_fnc_attachHeadphonesAction;
        };
		if(!_shift && _alt && _ctrlKey && playerSide isEqualTo civilian && license_civ_rebel && {!isNull cursorObject} && {cursorObject isKindOf "Man"} && {(isPlayer cursorObject)} && {(side cursorObject in [civilian,independent,west])} && {alive cursorObject} && {cursorObject distance player < 3.5} && {!(cursorObject getVariable "Escorting")} && {(cursorObject getVariable "restrained")} && {speed cursorObject < 1}) then {
            [] call life_fnc_attachGunnysackAction;
        };
    };
	...
	case 79: {
		...
		if (_shift) then
        {
			...
			{	
				[player,"AmovPercMstpSnonWnonDnon_exercisekneeBendB",0] remoteExec ["life_fnc_moveMnts",RANY];
			};
		};
    };
	...
	case 80: {
		...
		if (_shift) then
        {
			if (vehicle player == player && !(player getVariable ["restrained", false]) && (animationState player) != "Incapacitated" && !life_istazed) then
			{	
				[player,"AmovPercMstpSnonWnonDnon_exerciseKata",0] remoteExec ["life_fnc_moveMnts",RANY];
			};
		};
    };
	...
	case 81: {
		...
		if (_shift) then
        {
			if (vehicle player == player && !(player getVariable ["restrained", false]) && (animationState player) != "Incapacitated" && !life_istazed) then
			{	
				[player,"AmovPercMstpSnonWnonDnon_exercisekneeBendA",0] remoteExec ["life_fnc_moveMnts",RANY];
			};
		};
    };
	...
	case 75: {
		...
		if (_shift) then
        {
			if (vehicle player == player && !(player getVariable ["restrained", false]) && (animationState player) != "Incapacitated" && !life_istazed) then
			{	
				[player,"gestureHi",2] remoteExec ["life_fnc_moveMnts",RANY];
			};
		};
    };
	...
	case 76: {
		...
		if (_shift) then
        {
			if (vehicle player == player && !(player getVariable ["restrained", false]) && (animationState player) != "Incapacitated" && !life_istazed) then
			{	
				[player,"gestureHiB",2] remoteExec ["life_fnc_moveMnts",RANY];
			};
		};
    };
	...
	case 77: {
		...
		if (_shift) then
        {
			if (vehicle player == player && !(player getVariable ["restrained", false]) && (animationState player) != "Incapacitated" && !life_istazed) then
			{	
				[player,"gestureHiC",2] remoteExec ["life_fnc_moveMnts",RANY];
			};
		};
    };
	...
	case 71: {
		if (vehicle player != player && !life_siren_active && (driver vehicle player) == player) then {
            ...
            if ((_veh getVariable "siren")) then {
                titleText [localize "STR_MISC_RadioOFF","PLAIN"];
                _veh setVariable ["siren",false,true];
				deleteVehicle Radio;
            } else {
                titleText [localize "STR_MISC_RadioON","PLAIN"];
                _veh setVariable ["siren",true,true];
				Radio = "Land_HelipadEmpty_F" createVehicle position _veh;
				Radio attachTo [_veh, [0, 0, 1] ]; 
                [_veh] remoteExec ["life_fnc_Radio",RCLIENT];
            };
        };
    };*/
	...
	case 72: {
		...
		if (_shift) then
        {
			...
			if (_curObject isKindOf "Man" && !(_curObject isKindOf "Animal") && alive _curObject) then
			{	
				[life_clero_ausweis,life_clero_cop_dienstnummer] remoteExecCall ["life_fnc_showAusweis",_curObject];
			};
		};
    };*/
