...
*/
private ["_from","_curObject","_isWater","_CrateModelNames","_crate","_fish","_animal","_whatIsIt","_handle"];
...
{
	if(player distance _x <= 25 && vehicle player != player) exitWith
	{
		createDialog "life_my_taxi_heli";
	}; 
}foreach _from;

_from = [[parkplatz_in_1_automat,parkplatz_in_1_schranke,parkplatz_out_1_automat,parkplatz_out_1_schranke]];
{
	if(player distance (_x select 0) <= 5 && vehicle player != player && (driver (vehicle player) isEqualTo player)) exitWith
	{
		private "_handle";
		_handle = [0,_x] spawn life_fnc_parkplatzsystem;
		waitUntil {scriptDone _handle};
	}; 
}foreach _from;
{
	if(player distance (_x select 2) <= 5 && vehicle player != player && (driver (vehicle player) isEqualTo player)) exitWith
	{
		private "_handle";
		_handle = [1,_x] spawn life_fnc_parkplatzsystem;
		waitUntil {scriptDone _handle};
	}; 
}foreach _from;

_from = [[monitor1,camto1,"rendertarget0",cam0]];
{
	if(player distance (_x select 0) <= 5) exitWith
	{
		[(_x select 0),(_x select 1),(_x select 2),(_x select 3)] call life_fnc_livefeed;	
	}; 
}foreach _from;
...
if (isPlayer _curObject && _curObject isKindOf "Man") then {
	...
	if ((_curObject getVariable ["restrained",false]) && !dialog && playerSide isEqualTo civilian && license_civ_rebel) then {
